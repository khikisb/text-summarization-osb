...
def ringkas_teks(teks_artikel, top_n=3):
    ...
    try:
        for i in range(min(top_n, len(kalimat_terurut))):  # Pastikan tidak melebihi panjang kalimat_terurut
            teks_ringkasan.append(" ".join(kalimat_terurut[i][1]))
    except IndexError:
        st.error("Tidak ada kalimat yang dihasilkan dalam proses ringkasan.")
